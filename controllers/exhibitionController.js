...
exports.updateExhbition = function (req, res) {
  Exhibition.find({ _id: req.body._id }, (err, exhibition) => {
    if (exhibition.moderator == req._id) {
      ...
      Exhibition.updateOne({ _id: req.body._id }, exhibition, (err) => {
        if (!err) {
          res
            .status(200)
            .send({
      ...
      res
        .status(403)
        .send({ success: false, message: "Unauthorized to update exhibition" });
    }
  });
};
...
exports.getExhibition = function (req, res) {
  Exhibition.findOne(
    { _id: "62572b2ead92d761abc7097d" },
    "stands event_name carpet_color display_screen sponsor_disc sponsor_cylinder hall_type sponsor_banners"
  )
    .populate({
      path: "stands",
      populate: {
        path: "exponent",
        select:
          "exponent.company_name exponent.website exponent.firstName exponent.lastName",
      },
    })
    ...
    .exec((err, result) => {
      if (!err) {
        if (result) {
          if (result.stands) res.status(200).send(result);
          else {
            res.status(200).send([]);
          }
        } else {
...
exports.getEntrance = function (req, res) {
  Exhibition.findOne({ _id: req.params.id }, "entrance webinar").exec(
    (err, result) => {
      if (!err) {
        webinar =
          result.webinar.purchased && result.webinar?.videos?.length != 0;
        res.status(200).send({ ...result.entrance, webinar: webinar });
      } else {
        res.status(400).send({ success: false, message: err });
      }
    }
...
exports.getVisitors = function (req, res) {
  Exhibition.findOne({ _id: req.params.id }, "visitors")
    .populate({
      path: "visitors",
      select: "firstName visitor.lastName visitor.phoneNumber visitor.email",
    })
    .exec((err, result) => {
      if (!err) {
        if (result) {
          if (result.visitors) res.status(200).send(result.visitors);
          else {
            res.status(200).send([]);
          }
        } else {
...
exports.getExhibitionForVisitor = function (req, res) {
  Exhibition.findOne(
    { _id: req.params.id },
    "event_name exhibition_start_date exhibition_end_date is_free"
  ).exec((err, result) => {
    if (!err) {
      if (result) {
        ...
        res
          .status(404)
          .send({ success: true, message: "Exhibition not found." });
      }
    } else {
      res.status(400).send({ success: false, message: err });
    }
...
exports.updateSponsorDiscCustom0 = async (req, res) => {
  Exhibition.findOne({ _id: req.exhibition }, async (err, exhibition) => {
    if (!exhibition)
      return res
        .status(404)
        .json({ status: false, message: "exhibition record not found." });
    else {
      const canvas = createCanvas(1024, 1024);
      const ctx = canvas.getContext("2d");
      loadImage(
        process.env.AWS_S3_ROOT_PATH +
          exhibition.sponsor_disc.texture_download_url
      )
        .then(async (image) => {
          ...
          loadImage(req.file.path).then(async (logo) => {
            ...
            res
              ...
              .send({
                success: true,
                message: "Disc Custom 0 has been updated successfully",
              });
          });
        })
        .catch((error) => {
        ...
        .finally(() => {
          if (req.file) fs.rmSync(req.file.path, { recursive: true });
        });
    }
  });
};
...
exports.getExhibitionById = function (req, res) {
  Exhibition.findOne(
    { _id: req.params.id },
    "stands event_name carpet_color display_screen sponsor_disc sponsor_cylinder hall_type sponsor_banners webinar"
  )
    .populate({
      path: "stands",
      populate: { path: "exponent", select: "exponent.company_name" },
    })
    .exec((err, result) => {
      if (!err) {
        if (result) {
        ...
        .then(async (image) => {
          ...
          loadImage(req.file.path).then(async (logo) => {
            ...
            res
              ...
              .send({
                success: true,
                message: "Disc Custom 1 has been updated successfully",
              });
          });
        })
        .catch((error) => {
        ...
        .then((image) => {
          ...
          loadImage(req.file.path).then(async (logo) => {
            ...
            res
              ...
              .send({
                success: true,
                message: "Disc Custom 3 has been updated successfully",
              });
          });
        })
        .catch((error) => {
...
exports.updateSponsorCylindre0 = (req, res) => {
  Exhibition.findOne({ _id: req.exhibition }, (err, exhibition) => {
    if (!exhibition)
      ...
      loadImage(
        process.env.AWS_S3_ROOT_PATH +
          exhibition.sponsor_cylinder.texture_download_url_0
      )
        .then((image) => {
          ctx.drawImage(image, 0, 0);
          loadImage(req.file.path).then(async (logo) => {
            ...
            res
              ...
              .send({
                success: true,
                message: "Sponsor Cylindre has been updated successfully",
              });
          });
        })
        .catch((error) => {
...
exports.updateSponsorCylindre1 = (req, res) => {
  Exhibition.findOne({ _id: req.exhibition }, (err, exhibition) => {
    if (!exhibition)
      ...
      loadImage(
        process.env.AWS_S3_ROOT_PATH +
          exhibition.sponsor_cylinder.texture_download_url_1
      )
        .then((image) => {
          ctx.drawImage(image, 0, 0);
          loadImage(req.file.path).then(async (logo) => {
...
exports.updateSponsorCylindre2 = (req, res) => {
  Exhibition.findOne({ _id: req.exhibition }, (err, exhibition) => {
    if (!exhibition)
      ...
      loadImage(
        process.env.AWS_S3_ROOT_PATH +
          exhibition.sponsor_cylinder.texture_download_url_2
      )
        .then((image) => {
          ctx.drawImage(image, 0, 0);
          loadImage(req.file.path).then(async (logo) => {
...
exports.updateSponsorCylindre3 = (req, res) => {
  Exhibition.findOne({ _id: req.exhibition }, (err, exhibition) => {
    if (!exhibition)
      ...
      loadImage(
        process.env.AWS_S3_ROOT_PATH +
          exhibition.sponsor_cylinder.texture_download_url_3
      )
        .then((image) => {
          ctx.drawImage(image, 0, 0);
          loadImage(req.file.path).then(async (logo) => {
...
exports.updateSponsorBanner0 = (req, res) => {
  Exhibition.findOne({ _id: req.exhibition }, (err, exhibition) => {
    if (!exhibition)
      ...
      loadImage(
        process.env.AWS_S3_ROOT_PATH +
          exhibition.sponsor_banners.texture_download_url_0
      )
        .then((image) => {
          ctx.drawImage(image, 0, 0);
          loadImage(req.file.path).then(async (logo) => {
            ...
            res
              ...
              .send({
                success: true,
                message:
                  "Sponsor Banner Custom 0 has been updated successfully",
              });
          });
        })
        .catch((error) => {
...
exports.updateSponsorBanner1 = (req, res) => {
  Exhibition.findOne({ _id: req.exhibition }, (err, exhibition) => {
    if (!exhibition)
      ...
      loadImage(
        process.env.AWS_S3_ROOT_PATH +
          exhibition.sponsor_banners.texture_download_url_1
      )
        .then((image) => {
          ctx.drawImage(image, 0, 0);
          loadImage(req.file.path).then(async (logo) => {
...
exports.updateSponsorBanner2 = (req, res) => {
  Exhibition.findOne({ _id: req.exhibition }, (err, exhibition) => {
    if (!exhibition)
      ...
      loadImage(
        process.env.AWS_S3_ROOT_PATH +
          exhibition.sponsor_banners.texture_download_url_2
      )
        .then((image) => {
          ctx.drawImage(image, 0, 0);
          loadImage(req.file.path).then(async (logo) => {
...
exports.updateSponsorBanner3 = (req, res) => {
  Exhibition.findOne({ _id: req.exhibition }, (err, exhibition) => {
    if (!exhibition)
      ...
      loadImage(
        process.env.AWS_S3_ROOT_PATH +
          exhibition.sponsor_banners.texture_download_url_3
      )
        .then((image) => {
          ctx.drawImage(image, 0, 0);
          loadImage(req.file.path).then(async (logo) => {
            ...
            res
              ...
              .send({
                success: true,
                message:
                  "Sponsor Banner Custom 3 has been updated successfully",
              });
          });
        })
        .catch((error) => {
...
exports.addWebniarVideo = (req, res) => {
  ...
  if (req.files.thumb)
    webinarVideo.thumbnail_download_url = req.files.thumb[0].key;
  Exhibition.updateOne(
    { _id: req.exhibition },
    { $push: { "webinar.videos": webinarVideo } }
  )
    .then(() => {
...
exports.updateWebniarVideo = (req, res) => {
  ...
  Exhibition.findOne(
    { _id: webinarId, "webinar.videos._id": videoId },
    async (err, doc) => {
      if (err) {
        console.log(err);
        ...
        const video = doc.webinar.videos.find(
...
exports.deleteWebinarVideo = async (req, res) => {
  ...
  try {
    ...
    const videoIndex = webinar.videos.findIndex(
...
exports.getStands = function (req, res) {
  const exhibitionId =
    req.role == "moderator" ? req.exhibition : req.params.exhibitionId;
...
exports.getVisitors = async (req, res) => {
  try {
    ...
    const distinctVisitors = await Exhibition.distinct("visitors", {
      _id: exhibition,
    });
    ...
    res
      ...
      .send({
        success: true,
        data: visitors,
        nbDocuments: distinctVisitors.length,
      });
  } catch (err) {
...
exports.getExhibitions = function (req, res) {
  Exhibition.find(
    {},
    "event_name exhibition_start_date exhibition_end_date hall_type webinar.purchased moderator"
  )
    .populate({
      path: "moderator",
      select: "firstName lastName email",
  ...
  Exhibition.aggregate(
    [
      ...
      },
      {
        $addFields: {
          tranche1: {
            $cond: [{ $eq: ["$tranche1", []] }, [{ tranche1: 0 }], "$tranche1"],
          },
          tranche2: {
            $cond: [{ $eq: ["$tranche2", []] }, [{ tranche2: 0 }], "$tranche2"],
          },
          tranche3: {
            $cond: [{ $eq: ["$tranche3", []] }, [{ tranche3: 0 }], "$tranche3"],
          },
          tranche4: {
            $cond: [{ $eq: ["$tranche4", []] }, [{ tranche4: 0 }], "$tranche4"],
          },
          tranche5: {
            $cond: [{ $eq: ["$tranche5", []] }, [{ tranche5: 0 }], "$tranche5"],
          },
          tranche6: {
            $cond: [{ $eq: ["$tranche6", []] }, [{ tranche6: 0 }], "$tranche6"],
          },
          tranche7: {
            $cond: [{ $eq: ["$tranche7", []] }, [{ tranche7: 0 }], "$tranche7"],
          },
        },
      },
      {
    ...
    function (err, results) {
      if (!err) {
        ...
        else res.status(200).send({ success: true, data: 0 });
      } else {
  ...
  Exhibition.aggregate(
    [
      ...
      },
      {
        $addFields: {
          male: {
            $cond: [{ $eq: ["$male", []] }, [{ male: 0 }], "$male"],
          },
          female: {
            $cond: [{ $eq: ["$female", []] }, [{ female: 0 }], "$female"],
          },
        },
      },
      {
    ...
    if (!err)
      Ticket.count({ exhibition: req.exhibition }, function (err, count) {
        if (!err)
          res
            .status(200)
            .send({ success: true, data: count * result.ticket_price });
...
exports.updateEntranceSponsorBanner0 = (req, res) => {
  Exhibition.findOne({ _id: req.exhibition }, (err, exhibition) => {
    if (!exhibition)
      ...
      loadImage(
        process.env.AWS_S3_ROOT_PATH +
          exhibition.entrance.sponsor_banners.texture_download_url_0
      )
        .then((image) => {
          ctx.drawImage(image, 0, 0);
          loadImage(req.file.path).then(async (logo) => {
            ...
            res
              ...
              .send({
                success: true,
                message:
                  "Entrance Sponsor Banner Custom 0 has been updated successfully",
              });
          });
        })
        .catch((error) => {
...
exports.updateEntranceSponsorBanner1 = (req, res) => {
  Exhibition.findOne({ _id: req.exhibition }, (err, exhibition) => {
    if (!exhibition)
      ...
      loadImage(
        process.env.AWS_S3_ROOT_PATH +
          exhibition.entrance.sponsor_banners.texture_download_url_1
      )
        .then((image) => {
          ctx.drawImage(image, 0, 0);
          loadImage(req.file.path).then(async (logo) => {
            ...
            res
              ...
              .send({
                success: true,
                message:
                  "Entrance Sponsor Banner Custom 1 has been updated successfully",
              });
          });
        })
        .catch((error) => {
...
exports.updateEntranceCubeScreen00 = (req, res) => {
  Exhibition.findOne({ _id: req.exhibition }, (err, exhibition) => {
    if (!exhibition)
      ...
      loadImage(
        process.env.AWS_S3_ROOT_PATH +
          exhibition.entrance.cube_screen.texture_download_url
      )
        .then((image) => {
          ctx.drawImage(image, 0, 0);
          loadImage(req.file.path).then(async (logo) => {
            ...
            res
              ...
              .send({
                success: true,
                message:
                  "Entrance Cube Screen Custom 0 has been updated successfully",
              });
          });
        })
        .catch((error) => {
            ...
            loadImage(req.file.path).then(async (logo) => {
              ...
              res
                ...
                .send({
                  success: true,
                  message:
                    "Entrance Cube Screen Custom 1 has been updated successfully",
                });
            });
          })
          .catch((error) => {
...
exports.getVisitorsForNetworking = (req, res) => {
  Ticket.aggregate([
    {
      $match: {
        exhibition: mongoose.Types.ObjectId(req.params.exhibition),
        sharedata: true,
      },
    },
    {
...
exports.getVisitorsForNetworkingCount = (req, res) => {
  Ticket.count({ exhibition: req.params.exhibition, sharedata: true }).exec(
    (err, result) => {
      if (!err) {
...
exports.deleteExhibition = async (req, res) => {
  try {
    ...
    const result = await Exhibition.findOne({ _id: exhibitionId })
      .populate({ path: "stands" })
      .exec();
    ...
    result.stands.forEach((stand) => {
      const { _id, tv, banner, menu, texture_download_url, logo_download_url } =
        stand;
      standsToDelete.push(_id);
      if (tv?.media_download_url)
        standFilesToDelete.push({ Key: tv.media_download_url });
      if (banner?.texture_download_url)
        standFilesToDelete.push({ Key: banner.texture_download_url });
      if (menu?.pdf_download_url)
        standFilesToDelete.push({ Key: menu.pdf_download_url });
      if (texture_download_url)
        standFilesToDelete.push({ Key: texture_download_url });
      if (logo_download_url)
        standFilesToDelete.push({ Key: logo_download_url });
    });
    ...
    if (display_screen?.purchased)
      exhibitionFilesToDelete.push({ Key: display_screen.media_download_url });
    if (sponsor_disc?.purchased)
      exhibitionFilesToDelete.push({ Key: sponsor_disc.texture_download_url });
    if (sponsor_cylinder?.purchased)
      exhibitionFilesToDelete.push(
        { Key: sponsor_cylinder.texture_download_url_0 },
        { Key: sponsor_cylinder.texture_download_url_1 },
        { Key: sponsor_cylinder.texture_download_url_2 },
        { Key: sponsor_cylinder.texture_download_url_3 }
      );
    if (sponsor_banners?.purchased)
      exhibitionFilesToDelete.push(
        { Key: sponsor_banners.texture_download_url_0 },
        { Key: sponsor_cylinder.texture_download_url_1 },
        { Key: sponsor_cylinder.texture_download_url_2 },
        { Key: sponsor_cylinder.texture_download_url_3 }
      );
    if (entrance?.sponsor_banners)
      exhibitionFilesToDelete.push(
        { Key: entrance.sponsor_banners.texture_download_url_0 },
        { Key: entrance.sponsor_banners.texture_download_url_1 }
      );
    if (entrance?.cube_screen)
      exhibitionFilesToDelete.push({
        Key: entrance.cube_screen.texture_download_url,
      });
    ...
    res
      .status(200)
      .send({ success: true, message: "stands deleted successfully." });
  } catch (err) {
    console.log(err);
    res.status(500).send({ success: false, message: err });
  }
...
exports.getMyTickets = (req, res) => {
  Exhibition.findById(req.exhibition, "is_free ticket_price", (err, data) => {
    if (err) res.status(400).send({ success: false, messge: err });
    else {
    ...
    if (err) res.status(400).send({ success: false, messge: err });
      ...
      Exhibition.updateOne({ _id: req.exhibition }, exhibition, (err, data) => {
        if (err) {
          ...
          res
            .status(200)
            .send({ success: true, message: "Updated successfuly." });
        }
      });
    }
  });
