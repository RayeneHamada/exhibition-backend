                  ...
                  if (err) {
                    ...
                    Visitor.findOneAndUpdate(
                      { _id: visitor._id },
                      {
                        ...
                        $push: {
                          tickets: ticketDoc._id,
                          exhibitions: ticketDoc.exhibition,
                        },
                      }
                    )
                      .then((err, result) => {
                        if (err);
                        Exhibition.findOne(
                          { _id: metadata.exhibition },
                          async (err, exhibitionDoc) => {
                            if (!err) {
                              {
                                if (exhibitionDoc) {
                                  try {
                                  ...
                                  const htmlContent = `
                                      <h3>Login : </h3><strong>${visitor.email}</strong>
                                      <br/>
                                  ...
                                  await sendEmail(
                                    visitor.email,
                                    "Coordonnées d'accces à XPOLAND",
                                    htmlContent
                                  );
                                } else {
                                  res
                            ...
                            if (!err) {
                              {
                                if (exhibition) {
                                  if (
                                    !exhibition.visitors.includes(visitor._id)
                                  )
                                    try {
                                      await Exhibition.updateOne(
                                        { _id: metadata.exhibition },
