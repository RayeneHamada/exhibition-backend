...
var texture = {
  XL: "stand_main_albedo.png",
  LL: "stand_left_albedo.png",
  LR: "stand_right_albedo.png",
  M: "stand_medium_albedo.png",
  S: "stand_small_albedo.png",
};
var banners = {
  XL: [
    "stand_main_top_01_albedo.png",
    "stand_main_top_02_albedo.png",
    "stand_main_top_03_albedo.png",
    "stand_main_top_04_albedo.png",
  ],
  LR: [
    "stand_lr_top_01_albedo.png",
    "stand_lr_top_02_albedo.png",
    "stand_lr_top_03_albedo.png",
  ],
  LL: [
    "stand_lr_top_01_albedo.png",
    "stand_lr_top_02_albedo.png",
  ...
  M: ["stand_medium_top_01_albedo.png", "stand_medium_top_02_albedo.png"],
};
...
exports.signup = function (req, res, next) {
  ...
  User.countDocuments({ email: user.email }, function (err, c) {
    if (c == 0) {
      ...
      res.status(403).json({
        success: false,
        message: "There is already a used using this e-mail",
      });
    }
  });
};
    ...
    const accessToken = jwt.sign(
      {
        ...
        email: user.email,
        exponent_exhibition: user.exponent.exhibition,
        moderator_exhibition: user.moderator.exhibition,
        stand: user.exponent.stand,
      },
  ...
  user.save(async (err, userDoc) => {
    if (!err) {
      ...
      exhibition.save(async (err2, exhibitionDoc) => {
        if (err2) {
          ...
          if (err2.code == 11000)
            res
              .status(422)
              .json({ success: false, message: "Duplicate event name found." });
          else res.status(400).send({ success: false, message: err2 });
        } else {
          ...
          try {
            ...
            // If the email is sent successfully, you can proceed with the rest of the code.
            // For example, you can log a success message or continue with the file uploads.
          } catch (err) {
          ...
          try {
            const fileContent = fs.readFileSync(
              "./ressources/entrance_sponsor_banner.png"
            );
            const eTag = crypto
              .createHash("md5")
              .update(fileContent)
          ...
          if (req.body.exhibition.sponsor_disc) {
            try {
              const fileContent = fs.readFileSync(
                "./ressources/exhibition_sponsor_disc.png"
              );
              const eTag = crypto
                .createHash("md5")
                .update(fileContent)
          ...
          if (req.body.exhibition.sponsor_cylinder) {
            try {
              const fileContent = fs.readFileSync(
                "./ressources/exhibition_sponsor_cylindre.png"
              );
              const eTag1 = crypto
                .createHash("md5")
                .update(fileContent)
          ...
          if (req.body.exhibition.sponsor_banners) {
            try {
              const fileContent = fs.readFileSync(
                "./ressources/sponsor_banner_albedo.png"
              );
              const eTag5 = crypto
                .createHash("md5")
                .update(fileContent)
          ...
          await User.updateOne(
            { _id: userDoc._id },
            { "moderator.exhibition": exhibitionDoc._id }
          );
      ...
      if (err.code == 11000)
        res
          .status(422)
          .json({ success: false, message: "Duplicate email adrress found." });
      else return next(err);
    }
  });
};
...
exports.createExponent = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Exponent created successfully." });
  } catch (error) {
...
exports.participate = (req, res) => {
  User.findOne(
    { "visitor.email": req.body.email, role: "visitor" },
    (err, user) => {
      if (!err) {
        if (user) {
          Exhibition.findOne(
            { _id: req.body.exhibition },
            (err, exhibition) => {
              if (!err) {
                if (exhibition.visitors) {
                  if (
                    exhibition.visitors.find((visitor) => {
                      return new mongoose.Types.ObjectId(user._id).equals(
                        visitor
                      );
                    })
                  )
          ...
          if (err) {
            ...
            User.findOneAndUpdate(
              { _id: visitor._id },
              {
                ...
                $push: { "visitor.tickets": ticketDoc._id },
              }
            )
              .then((err, result) => {
                if (err);
                    ...
                    if (!err) {
                      {
                        if (exhibition) {
                          ...
                          try {
                            let info = await transporter.sendMail({
                              ...
                              html:
                                ...
                                "</strong><br/><h3>Password : </h3><strong>" +
                                password +
                                '</strong><br/><h2 style="color:red;">NB : Veuillez changer votre mot de passe lors de votre première connexion</h2>', // html body
                            });
                            res.status(201).send({
                              success: true,
                          ...
                          res
                            .status(404)
                            .send({ error: "Exhibition not found" });
                        }
                      }
                    } else res.send(err);
                  }
...
exports.authenticateVisitor = (req, res, next) => {
  User.findOne({ email: req.body.email }, (err, doc) => {
    if (!err) {
      if (doc) {
        Ticket.find(
          { visitor: doc._id, exhibition: req.body.exhibition },
          (err, docs) => {
            if (!err) {
              if (docs.length > 0) {
                passport.authenticate("local", (err, user, info) => {
                  ...
                  // registered user
                    ...
                    return res
                      .status(200)
                      .json({ success: true, token: user.generateJwt() });
                  // unknown user or wrong password
                  else return res.status(404).json(info);
                })(req, res, next);
              } else {
                res
                  .status(403)
                  .send({ success: false, message: "No ticket found." });
              }
            }
          }
        );
    ...
    if (err) res.status(500).send({ success: false, message: err });
      ...
      if (user) {
        ...
        try {
                                                ...
                                                <td colspan="2" style=" padding: 0 70px;/*background-color: #f8a44b;*/ text-align: center;">
                                                    <p style="font-family: 'arial', sans-serif; font-size: 16px; font-weight: 400; line-height: 22px">
                                                        <a href="${
                                                          ...
                                                          "reset-password/" +
                                                          token
                                                        }" target="_blank" style="text-decoration:none;background: #5d33ce;color: #fff;font-size: 14px;padding:17px;text-transform: uppercase;-webkit-border-radius: 30px;-moz-border-radius: 30px;-o-border-radius: 30px;-ms-border-radius: 30px;border-radius: 30px;line-height: 1.2;white-space: normal;width: 170px;border: 1px solid transparent;font-weight: 500;">Changer votre mot de passe</a>
                                                    </p>
                                                </td>
                                            </tr>
                                            <tr>
          ...
          res
            .status(200)
            .send({ success: true, message: `E-mail sent successfully.` });
        } catch (err) {
          res.status(500).send({
            success: false,
            message: `Le mail n'a pa pu être envoyé.`,
          });
        }
      } else {
